...
const OdissePdf = (() => {
  ...
  const PRETO = [0, 0, 0];
  const LINHA_COR = [127, 103, 95]; // cor de apoio só para linhas/divisores, nunca para texto

  const TOPO_CONTEUDO = 118;   // onde o corpo do texto pode começar, abaixo do cabeçalho
  const FUNDO_LIMITE = 738;    // limite antes de quebrar página — deixa espaço livre até o rodapé
  const RODAPE_Y = 762;

  // ---------- estilos ----------
  function estiloNomeCliente(doc) { doc.setFont('helvetica', 'bold'); doc.setFontSize(12.5); doc.setTextColor(...PRETO); }
  function estiloNumero(doc) { doc.setFont('helvetica', 'bold'); doc.setFontSize(10.5); doc.setTextColor(...PRETO); }
  function estiloTitulo(doc) { doc.setFont('helvetica', 'bold'); doc.setFontSize(10.5); doc.setTextColor(...PRETO); }
  function estiloCorpo(doc, tamanho = 10) { doc.setFont('helvetica', 'normal'); doc.setFontSize(tamanho); doc.setTextColor(...PRETO); }
  function estiloCorpoBold(doc, tamanho = 10) { doc.setFont('helvetica', 'bold'); doc.setFontSize(tamanho); doc.setTextColor(...PRETO); }
  function estiloRodapePequeno(doc) { doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5); doc.setTextColor(...PRETO); }
  function estiloRodapePequenoBold(doc) { doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5); doc.setTextColor(...PRETO); }
  ...
  function gerar(state, data) {
    ...
    for (const etapa of etapasSel) {
      ...
      y += 13;
      y = paragrafo(doc, etapa.descricao, LEFT + 8, y, CONTENT_W - 8, 9.5);
      y += 4;
    }
    ...
    if (state.dataInicio && etapasSel.length) {
      ...
      doc.autoTable({
        ...
        styles: { fontSize: 8.5, textColor: PRETO, cellPadding: 5 },
        headStyles: { fillColor: PRETO, textColor: 255 },
    ...
    if (r) {
      ...
      doc.autoTable({
        ...
        styles: { fontSize: 9.5, textColor: PRETO, cellPadding: 6 },
        columnStyles: { 1: { halign: 'right' } },
        didParseCell: (d) => {
          if (d.row.index === distrib.length) { d.cell.styles.fontStyle = 'bold'; d.cell.styles.fillColor = [242, 242, 242]; }
        }
      });
      ...
      y = paragrafo(doc, `À vista: ${OdisseCalc.fmtMoeda(plano.avista)} (desconto de ${Math.round((state.descontoAvista || 0) * 100)}%)`, LEFT, y, CONTENT_W, 9.5);
      y += 10;

      estiloCorpoBold(doc, 9.5);
    ...
    if (state.incluirDespesasReembolsaveis) {
      ...
      y = paragrafo(doc, 'O cliente deverá providenciar o reembolso em até 30 dias, contados do recebimento dos respectivos comprovantes.', LEFT, y, CONTENT_W, 9.5);
    }
    ...
    if (assessSelecionados.length) {
      ...
      y = paragrafo(doc, data.assessoramento.descricao_abertura, LEFT, y, CONTENT_W, 9.5);
      y += 6;
